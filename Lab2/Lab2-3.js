...
window.onload = function init() {
    ...
    // Effect speed
    ele = document.getElementById("speedid");
    ele.onchange = ele.oninput = function(event) {
        effectSpeed = (event.srcElement || event.target).value;
        document.getElementById("speedval").innerHTML = effectSpeed;
    };
    ele.value = effectSpeed;
    document.getElementById("speedval").innerHTML = effectSpeed;
...
function render()
{
    ...
    timeUn=0.0; //use this for a waving like effect(preferably 0 to 1 range)
    scaleFactor+=Number(effectSpeed); //displacement amount, set to a static value for proper waving effect. Now we need a small increment for this part
    frequency=0.0;//use this with timeUn for a waving like effect
