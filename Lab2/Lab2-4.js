...
window.onload = function init() {
    ...
    angleDegreesLoc = gl.getUniformLocation(program, "angle_degrees");
    ...
    //Twist values
    ele = document.getElementById("twistdegrees");
    ele.onchange = ele.oninput = function(event) {
        angleDegrees = Number((event.srcElement || event.target).value);
        document.getElementById("degval").innerHTML = angleDegrees;
    };
    ele.value = angleDegrees;
    document.getElementById("degval").innerHTML = angleDegrees;

    //Twist factor
    ele = document.getElementById("twist");
    ele.onchange = ele.oninput = function(event) {
        twistFactor = Number((event.srcElement || event.target).value);
        document.getElementById("twistval").innerHTML = twistFactor;
    };
    ele.value = twistFactor;
    document.getElementById("twistval").innerHTML = twistFactor;
...
function render()
{
    ...
    //twistFactor=1.0; //the higher the number the lesser the effect
    //angleDegrees=-90.0; //twist angle in degrees
    ...
    gl.uniform1f(factorLoc, twistFactor);
    gl.uniform1f(angleDegreesLoc, angleDegrees);
