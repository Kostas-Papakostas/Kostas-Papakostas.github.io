...
var lightPosition=new vec4(0, 2, 1, 1);
...
function UIInit() {
    ...
    ele.onchange = ele.oninput = function (event) {
        lightRotationY = (event.srcElement || event.target).value;
        document.getElementById("directionval").innerHTML = lightRotationY;
    };
    ele.value = lightRotationY;
    document.getElementById("directionval").innerHTML = lightRotationY;

    // Specular shininess slider
    var ele = document.getElementById("shininess");
    ele.oninput = ele.onchange = function (event) {
        shininess = (event.srcElement || event.target).value;
...
function mainProgramInit() {
    ...
    for (var i = 0; i < nLights; i++) {
        ...
        if (i == 0) {
            ...
            light[0].pos = lightPosition;
            light[0].rotY=lightRotationY;
...
function render()
{
    ...
    //gl.uniform4fv(light[0].position, mult(transpose(mv), light[0].pos), vec4(1,1,0,0));
    //var currentPos = mult(transpose(mv),light[o].pos);
    //var currentRot = mult(currentPos, rotate(light[0].rotY,vec3(0,1,0));
    gl.uniform4fv(light[0].position, mult(transpose(mv), mult(rotate(lightRotationY,vec3(0,1,0)),vec4( 1,1,0,0))));
...
function shaderSelected(){
    var radios = document.getElementsByName('shading');
