...
window.onload = function init() {
    ...
    frequencyLoc = gl.getUniformLocation(program, "frequency");
    ...
    //displacement amount slider
    ele = document.getElementById("amountdis");
    ele.onchange = ele.oninput = function(event) {
        scaleFactor = (event.srcElement || event.target).value;
        document.getElementById("amountval").innerHTML = scaleFactor;
    };
    ele.value = scaleFactor;
    document.getElementById("amountval").innerHTML = scaleFactor;

    // time interval
    ele = document.getElementById("time");
    ele.onchange = ele.oninput = function(event) {
        timeInterval = (event.srcElement || event.target).value;
        document.getElementById("timeval").innerHTML = timeInterval;
    };
    ele.value = timeInterval;
    document.getElementById("timeval").innerHTML = timeInterval;

    // frequency slider
    ele = document.getElementById("frequency");
    ele.onchange = ele.oninput = function(event) {
        frequency = (event.srcElement || event.target).value;
        document.getElementById("frequencyval").innerHTML = frequency;
    };
    ele.value = frequency;
...
function render()
{
    ...
    timeUn+=Number(timeInterval); //set to 0 for static effect, won't be used for this part
    //scaleFactor=0.2; //displacement amount, we only need this for now
    //frequency=0.0;//set to 0 for static effect, won't be used for this part

    gl.uniform1f(timeLoc,timeUn);
    gl.uniform1f(frequencyLoc,frequency);
    gl.uniform1f(scaleFactorLoc,scaleFactor);
